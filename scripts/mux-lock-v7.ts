...
const TMP = resolve(ROOT, "lock-v7-tmp");
const OUT = resolve(ROOT, "drama-lock-v7.mp4");
const FAIL_OUT = resolve(ROOT, "drama-lock-v7-audit-fail.mp4");
const AUDIT = resolve(ROOT, "drama-lock-v7-audit.json");
const PRIOR = 61.81;
const SPEND = 0;
const SLATE_S = 0.3;
/**
 * First mux used founder 1.0s lead minus 0.3s first-frame hold (pad 0.7).
 * Picture then showed the speaking viseme ~1s before voice (mouth then voice).
 * One remux: drop Eli pad to 0 (take is already onset-aligned; Mara stays 0).
 */
const ELI_AUDIO_PAD_S = 0;
const ELI_FIRST_MOTION_S = 0;
...
async function lockV7Slate(dest: string): Promise<void> {
...
async function main() {
  ...
  // Drop the 0.3s identity still at the head. Video+audio trimmed together so
  // take sync is unchanged (do not viseme-pad; a 1.0s pad overshot).
  await run("ffmpeg", [
    "-y", "-ss", "0.300", "-i", eliSrc,
  ...
  const eliPad = ELI_AUDIO_PAD_S;
  process.stdout.write(
    `  take mara voice=${maraVoice} mouth=${maraMouth} pad=0 · eli voice=${eliVoice} mouth=${eliMouthOnTake} in=${ELI_FIRST_MOTION_S} pad=${eliPad} (pad reduced after overshoot)\n`,
  );
  ...
  const eliPictureOut = Math.max(4, eliOut - ELI_FIRST_MOTION_S);
  ...
  const manifest: RenderManifest = {
    ...
    shots: [
      ...
      },
      {
        ...
        in_point_seconds: ELI_FIRST_MOTION_S, out_point_seconds: eliOut,
        picture_start_seconds: maraOut, audio_start_seconds: maraOut,
        hold_tail_seconds: 0, scene_kind: "dialogue", transition_in: "cut",
        audio_role: "onscreen", heard_audio: "native",
      },
    ],
  ...
  const mixed = await assembleEpisodeMp4({
    ...
    vtt: [
      ...
      "",
      "2",
      `00:00:${maraOut.toFixed(3).padStart(6, "0")} --> 00:00:${(maraOut + 3).toFixed(3).padStart(6, "0")}`,
  ...
  const open0 = resolve(dir, "open-0.00.png");
  const open02 = resolve(dir, "open-0.20.png");
  const afterSlate = resolve(dir, "open-after-slate.png");
  await dumpFrame(slated, 0.05, open0);
  await dumpFrame(slated, 0.2, open02);
  await dumpFrame(slated, SLATE_S + 0.05, afterSlate);
  const stillPortraitOpen = false;
  ...
  for (const [i, window] of windows.entries()) {
    ...
    if (lagMs != null && lagMs < -200) overshoot = true;
  ...
  const maraA = resolve(dir, "mara-head-a.png");
  const maraB = resolve(dir, "mara-head-b.png");
  await dumpFrame(slated, SLATE_S + 0.05, maraA);
  await dumpFrame(slated, SLATE_S + 0.35, maraB);
  const maraMotion = await grayMeanAbs(maraA, maraB);
  ...
  const report = {
    ship,
    reason: ship
      ? "No name-plant stills. Text slate only. Same Mara take (pad 0). Eli same take; 1.0s pad overshot (mouth then voice) so remuxed at pad 0. Native audio."
      : "Frame-audit failed. Do not treat this as the watch file.",
    path: ship ? OUT : FAIL_OUT,
    watch_file: ship ? OUT : null,
    do_not_overwrite: ["assets/drama-lock-v6.mp4", "assets/drama-lock-slice.mp4"],
    duration_seconds: Number(duration.toFixed(3)),
    sha256,
    lines,
    frames,
    sheer_or_bra: sheer,
    still_portrait_open: stillPortraitOpen,
    name_plants: false,
    overshoot_mouth_then_voice: overshoot,
    mara_head_motion: Number(maraMotion.toFixed(3)),
    takes: {
      ...
      eli_first_motion_s: ELI_FIRST_MOTION_S,
      eli_audio_pad_s: eliPad,
      eli_pad_reduced_after_overshoot: true,
    },
    ...
    soft_cap_usd: 5,
    product: {
      sync: "aligned ≤150ms pad 0; voice-lead ≥400ms pads that delay, clamp 0.4–1.5s; MixInput.visemePadSeconds per-take override",
      cut: "name-plant stills removed; speakers identified by NAME: line captions on moving CUs",
    },
  };
  await writeFile(AUDIT, JSON.stringify(report, null, 2));
  if (ship) {
    await writeFile(OUT, body);
    process.stdout.write(`LOCK V7 PASS ${OUT} · ${duration.toFixed(2)}s · sha256 ${sha256}\n`);
    return;
  }
  await writeFile(FAIL_OUT, body);
  process.stdout.write(`LOCK V7 FAIL ${FAIL_OUT} · ${duration.toFixed(2)}s · sha256 ${sha256}\n`);
  process.stdout.write(`${JSON.stringify({ lines, overshoot, sheer, noNamePlants, maraMotion }, null, 2)}\n`);
  process.exit(1);
}
