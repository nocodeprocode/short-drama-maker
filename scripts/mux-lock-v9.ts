...
const TMP = resolve(ROOT, "lock-v9-tmp");
const OUT = resolve(ROOT, "drama-lock-v9.mp4");
const FAIL_OUT = resolve(ROOT, "drama-lock-v9-audit-fail.mp4");
const AUDIT = resolve(ROOT, "drama-lock-v9-audit.json");
const PRIOR = 61.81;
const SPEND = 0;
const SLATE_S = 0.3;
/** v8 cut at 1.8s; founder still saw ~1s of room slide. +1.2s → 3.0s on the take. */
const MARA_SETTLE_S = 3.0;
...
async function lockV9Slate(dest: string): Promise<void> {
...
async function main() {
  ...
  await dumpFrame(resolve(SRC, "mara-try1.mp4"), 3.4, settled);
  const later = resolve(dir, "open-after-slate-plus.png");
  await dumpFrame(slated, SLATE_S + 0.35, later);
  const vsIdentity = await grayMeanAbs(afterSlate, identity);
  const vsSettled = await grayMeanAbs(afterSlate, settled);
  const headStep = await grayMeanAbs(afterSlate, later);
  const maraSettledOpen = vsIdentity > 16 && vsSettled < 28 && headStep < 16;
  ...
  const report = {
    ship,
    reason: ship
      ? "Mara CU starts after the remaining ~1s room slide (in_point 3.0s). Picture and native audio trimmed by the same 3.0s. Eli unchanged from v8. Sync kept."
      : "Frame-audit failed. Do not treat this as the watch file.",
    path: ship ? OUT : FAIL_OUT,
    watch_file: ship ? OUT : null,
    do_not_overwrite: ["assets/drama-lock-v8.mp4", "assets/drama-lock-v7.mp4", "assets/drama-lock-v6.mp4"],
    ...
    mara_head_step: Number(headStep.toFixed(3)),
  ...
  if (ship) {
    ...
    process.stdout.write(`LOCK V9 PASS ${OUT} · ${duration.toFixed(2)}s · sha256 ${sha256}\n`);
    return;
  }
  await writeFile(FAIL_OUT, body);
  process.stdout.write(`LOCK V9 FAIL ${FAIL_OUT} · ${duration.toFixed(2)}s · sha256 ${sha256}\n`);
  process.stdout.write(`${JSON.stringify({ lines, vsIdentity, vsSettled, headStep, sheer, noNamePlants }, null, 2)}\n`);
  process.exit(1);
}
