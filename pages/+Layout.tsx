...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  if (isAuth) {
    return (
      <RouteProvider>
        <div className="min-h-dvh bg-secondary_alt text-primary">{children}</div>
      </RouteProvider>
    );
  }
  ...
  if (isLegal) {
    ...
    return (
      <RouteProvider>
        <div className="flex min-h-dvh flex-col bg-primary text-primary">
          <header className="flex h-16 items-center justify-between gap-4 px-4 sm:px-6">
            <a href="/" className="rounded-md text-md font-semibold text-primary outline-focus-ring">
              {LEGAL_ENTITY.product_name}
            </a>
