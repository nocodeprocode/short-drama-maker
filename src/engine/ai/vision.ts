...
close is true only if the head fills at least a third of the frame height and the picture stops at the chest or above. Judge it by what the bottom edge cuts: a still that reaches the waist, the hips, or the bottom of a jacket is NOT close, however readable the eyes are, and a fashion or lookbook mid-shot is NOT close.
modest is true if clothes are on and opaque AND the chest is covered to the collarbone. It is false when an open jacket, blazer, or shirt is worn over bare skin with no garment beneath, when a neckline plunges past the collarbone, or when cleavage or a bare sternum is visible.
